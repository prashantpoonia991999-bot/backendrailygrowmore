...
async def download_user_panel():
    """Download User Panel files for Hostinger"""
    path = ROOT_DIR / "static" / "user-panel-hostinger.zip"
    if not path.exists():
        raise HTTPException(status_code=404, detail="User panel not found")
    return FileResponse(path=str(path), filename="user-panel-hostinger.zip", media_type="application/zip")

# Download Admin Panel for Hostinger
@api_router.get("/download-admin-panel")
async def download_admin_panel():
    """Download Admin Panel files for Hostinger"""
    path = ROOT_DIR / "static" / "admin-panel-hostinger.zip"
    if not path.exists():
        raise HTTPException(status_code=404, detail="Admin panel not found")
    return FileResponse(path=str(path), filename="admin-panel-hostinger.zip", media_type="application/zip")
